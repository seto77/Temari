...
- **現状の残務・次の一手の正本 = `docs/next_phase_2026-08-07.md`**
  (`..._2026-08-06.md` は P1 完了時点の旧版)。次の一手は **KLI を Dirac 経路へ広げる**
  こと (同書 §2′)。非相対論への配線は完了済み
...
- **2026-08-07: 厳密交換 (KLI) を SCF へ配線** (`--kli`)。局所交換の α という knob と
  Latter 補正の両方が消え、尾 −(Z−N+1)/r は物理の帰結になる。f_x/f_e は α=1 に対して
  3–15 倍改善し、**「最良の α」0.75 よりも良い** (診断書 §5 が測定の正本)。
  ただし**非相対論 SCF にしか配線していない**ので、重元素は相対論的収縮が勝つ →
  **出荷既定は Dirac + Xα のまま**。既定処方は 5 チャネル === ビット同一で無傷
