...
- QC は `tools/check_tables.jl` (Julia 版。C1-C8 + **C9 = 軌道の割り当て** +
  ...
  **E0 軸の両端 = 閾値直上と 400 kV 側が構造的に死角**。区間の内側を直接計算すると
  Sn K @閾値直上で **2.606e-03** に対し C6 は 8.392e-04 = **3.1 倍過小**、
  一方 Rn L2 では C6 が 4.0 倍**保守的**。**上界でも下界でもない**。
  区間内の直接測定は **`tools/e0_interp_probe.jl`**。
  C6b (外側 2 ノード込み) は**記録のみでゲートにしない** — ゲート 5e-3 の余裕が
  4.2 倍 → 1.02 倍に潰れるうえ、代理量として 1.9 倍過大だから
