...
- **現状の残務・次の一手の正本 = `docs/next_phase_2026-08-07.md`**
  (`..._2026-08-06.md` は P1 完了時点の旧版)。次の一手は **(A) 厳密 frozen core の実装**
  (同書 §2 — GOS の数 % の本丸) と **(B) 横断的 Møller 相互作用** (同書 §3 — σ 自前化の
  最安の一手) の並行。出荷既定を Dirac+KLI にするかは実装済みで**作者判断待ち** (§4)
...
- **⚠ 公開前の法務確認**: `src/bote_salvat.json` は公開係数表そのもので、
  「係数をリポに入れない」方針と矛盾する。出所 (論文 or PENELOPE) の確認が要る
