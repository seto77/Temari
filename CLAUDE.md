...
- **⚠⚠ 2026-08-07 の最重要発見: SRC (出荷 v3 の連続状態) が 1.5–6 % ずれている。**
  連続状態の処方だけを 3 通り比べると、**真の相対論補正 (kdirac − 非相対論) は
  s ≤ 1.25 で 0.3 % 以下**なのに、**SRC は非相対論から 1.5–6 % 離れる**
  (Fe s=1.25 で +1.65 %)。本番求積でも同値、規格化規約でもない。
  外部参照との一致も **Z ≤ 26 では非相対論 (v2) の方が SRC (v3) より良い**。
  ⇒ **v2 → v3 の SRC 採用は軽・中元素では改良でなかった可能性**。
  kdirac は非相対論のすぐ隣に着地するので v4 で自動解消する。
  詳細と反証の可能性は `docs/fs_external_validation_2026-08-07.md` の末尾
