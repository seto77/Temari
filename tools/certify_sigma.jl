...
`docs/notes/sigma_beta_delta_contract_2026-08-18.md` §5A の `observed_max` を、代表標本
...
⚠ **同じ変数・同じ求積族の自己収束は使わない** — `docs/notes/beta_spike_2026-08-18.md` §2 で
...
(a) このホストのビット化け (`docs/notes/host_stability_2026-08-19.md`) か
...
function report_duplicates(dups)
    ...
    if exact < n
        ...
        if !isempty(same_state)
            ...
            println("     非決定性か、ホストのビット化け (`docs/notes/host_stability_2026-08-19.md`)")
        end
...
**同じ入力を別のプロセス・別の時刻で計算した対**なので、
  ...
  - **ホストのビット化け** (`docs/notes/host_stability_2026-08-19.md` の仮説)
...
(`docs/notes/host_stability_2026-08-19.md`)。**散発的なビット化けが起きているなら、
