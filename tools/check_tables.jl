...
検査項目 (C1-C8 は Python 版と同じ意味):
  ...
  C6  E0 ノードを 1 つ抜いて PCHIP(ln(u−1)) を再構築し、抜いた点での誤差を測る
      ...
      **E0 軸の両端 = 閾値直上と最高電圧側が構造的に死角**。区間の内側を直接計算した
      50 チャネル 450 点の層別標本で、**危険域 (u_min<2) の 25 本中 20 本で C6 が下回った**
      (最悪の直接測定は Po L1 の **3.00e-03**、C6 は 5.11e-04)。
      **破損検出器であって保証ではない**。区間内の直接測定は `tools/e0_interp_probe.jl sweep`
  C6b 外側 2 ノードも抜いた LOO。**記録のみ・ゲートにしない** (理由は
      `c6_edge_worst` の docstring — **C6b も 4/50 で直接測定を下回り**、比が 0.33〜15.8 と
      48 倍ばらつく。加えてゲート 5e-3 の余裕が 4.2 倍 → 1.02 倍に潰れる)
...
`tools/e0_interp_probe.jl sweep` が区間の内側を**直接計算**して測った実測
(50 チャネル・450 点の層別標本。指示書 §2 P1b):

| 層 | n | 直接測定 中央 / p90 / 最大 | C6 ≥ 直接 | C6b ≥ 直接 |
|---|---|---|---|---|
| risk (u_min<2 の**全数**) | 25 | 9.63e-04 / 2.44e-03 / **3.00e-03** | **5/25** | 22/25 |
| shell (L3・M 殻・軽元素) | 9 | 1.95e-05 / 8.47e-05 / 8.47e-05 | 9/9 | 9/9 |
| c6b (代理量の上位) | 8 | 4.17e-04 / 1.52e-03 / 1.52e-03 | **1/8** | 7/8 |
| ctrl (対照) | 8 | 9.43e-06 / 8.56e-05 / 8.56e-05 | 8/8 | 8/8 |

⇒ **C6 は上界でも下界でもない。**内側のノードでは区間が 2 倍になるので過大に、
端では見ていないので過小に出る。**危険域では 25 本中 20 本で下回った。**

⚠⚠ **C6b も上界ではない。**k=2 まで広げても **4/50 で直接測定を下回り**、
比は最小 **0.33** から最大 15.8 まで 48 倍にばらつく (中央 3.12)。
**代理量として較正されていないので、ゲートにも経験的な上界にもできない。**
加えて k=2 まで広げると全 525 チャネルの最悪が 1.183e-03 → **4.916e-03** になり、
ゲート 5e-3 に対する余裕が 4.2 倍 → **1.02 倍**に潰れる。ばらつく代理量を
1.02 倍の余裕でゲートにすれば、次世代で正常な行を「破損」と判定する
(C2 の窓を 10.0 にしなかったのと同じ判断)。**記録して見えるようにするだけ**にする。

⚠ 危険は**閾値側の第 1 区間に集中する** — 直接測定の最悪は 50 本中 40 本が第 1 区間で、
400 kV 側の最終区間は 2 本しかない。両端が等しく危ないのではない。"""
