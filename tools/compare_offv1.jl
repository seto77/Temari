...
⚠⚠ **参照ファイルはリポジトリに入っていない** (`refs/` は索引だけ追跡)。
  ...
  そのものを一切出力しない** — 出すのは差の集計と合否だけ。個々の点の差は
  `--verbose` でだけ出す (既定 off は出力量の都合)。
  ★ 2026-08-17 作者決定: **点ごとの差 (比) は公開してよい** — CONTRIBUTING の線は
  「比・乖離は可、転記は不可」で、docs サイトの `tools/make_comparison_figures.py` は
  実際に点ごとの偏差を図にして載せている。「ours が公開されているので点ごとの差から
  参照が復元できる」という以前の慎重論は取り下げた。
