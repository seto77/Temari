...
## 捕獲後の知見 (2026-08-06, run e8_20260806_103602)

- **捕獲**: Z6 K E0275、w01 (同一プロセス) の pass_0001 vs pass_0025。判定 (a) =
  ε ノード 67/96 の 1 スライスのみ相違。eps/we 一致・N0 一致・jt/bt 一致。
  N 相違 33/161 (|dULP|≤2)、F 相違 29/161 (|dULP|≤3、全て小 F の尾側)
- **整列は無相関**: 非フリップ 101 パスの整列は dNde mod4096 で 15 種・we mod64
  で 4 種に散らばり全て同一ビット。フリップの整列タプル (0,1088,48,32) に
  近い整列の非フリップも多数 → 外側配列整列はヒープ状態のプロキシに過ぎない
- **単一ノード再現 (tools/e8_replay.jl)**: 捕獲 ref スライスをビット完全再現
  (ハーネス忠実性確認)。ヒープ撹乱 t1×40・併走 t4×200 の全てで 1 変種 =
  **決定的な文脈撹乱 (整列・併走・割り当てノイズ) では割れない**
- 帰結: 残る整合的機構は「実行中 1 回だけ起きる過渡的な状態擾乱」= GC
  (concurrent sweep) 系のメモリ効果が筆頭。BLAS 縮約 (判定 b) は今回の捕獲では
  シロ (gemv はスライス 67 の差をそのまま伝搬しただけ)
