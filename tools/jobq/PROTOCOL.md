...
- ⚠⚠ **RETURN した票は、その worker だけが `degraded_sleep` のあいだ飛ばす**。RETURN は票を
  **同じ名前で** `queue/` に戻すので、記憶しないと `claim` は名前順の先頭にあるその票を毎回また取る。
  ホストではなく**票か共有の側**に理由がある RETURN (典型は「書庫が共有に置き忘れられている」) では、
  全ワーカーが同じ票を取っては戻すだけになり、**その後ろの票に永久に到達しない** — 名前順が先の
  campaign が丸ごと詰まっていれば、フリート全体が別の campaign に着手できない。
  ⚠ 忘れるのは**自分だけ**で、共有には何も書かない — 他のホストには試させる (それが RETURN の趣旨で、
  ホスト固有の障害はそれで解ける)。期限が切れれば同じ worker も試し直すので、置き忘れが直れば自然に回復する。
