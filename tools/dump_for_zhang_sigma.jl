...
⚠ 出力は **JSONL** (1 行 = 1 チャネル)。理由は 2 つ:
  - ホストが不安定で長い実行が落ちる (`docs/notes/host_stability_2026-08-19.md`)。
...
function dump_entry(elem::String, z::Int, tag::String, zedge::String, e0::Float64;
    ...
    #
    # ⚠⚠ 理由: Python 側で σ を組むとき、窓 [0, Δ₂] の √ε GL ノードは最小 ~1e-4 eV
    # まで落ちるので、下端 0.2 eV では**面の外を clamp していた**。回数ではなく重みで
    # 測ると、σ の **4.8e-03** ([0,50] eV 窓、Fe K) を外挿点が占めていた
    # (参照 (Zhang) 側は下端 0.01 eV なので同じ条件で 1.2e-04)。
    #
    # ⚠ **これは「重み」であって「誤差」ではない。**格子を伸ばす前と後で σ 比は
    # **4 桁一致する** (1.1083 → 1.1083)。伸ばしたのは比が動くからではなく、
    # **外挿点をゼロにして CLAMP_MAX 門を自明に通し、論点を減らすため**である。
    # 詳細と訂正の経緯 = `docs/notes/external_gate_2026-08-19.md` §3.1。
