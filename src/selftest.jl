# 自己検証 — 解析解に対するテストの梯子 (T0-T24 と T26-T27) と Python 参照値との照合
#
...
# ⚠ 260818Cl 訂正: この前書きは T8 までしか無かった頃のもの。(1) 梯子は
#   T0-T24 と T26-T27 (T25 は欠番) まで伸びており、各テストの正本は直上の
#   コメント。(2) T0c だけは外部の高精度値ではなく in-process の BigFloat
#   参照と照合する。(3) 上の「実 c の効果は物理」は誤り — 詳細は T8 の直上。
