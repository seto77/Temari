...
#
# 係数 1/(8π² a₀) = 0.0239337 Å (260818Cl 訂正: 0.023937 は誤記) は
# 結晶学で使われる 0.023934 と一致する
...
#   * f_e は第 1 Born。低速電子や重元素の大角では歪曲波 (Mott 断面積) でないと
#     足りない — 260818Cl 訂正: それは `mott` 出口 (l5_exit_mott.jl) で実装済。
#     ただし `phase` 出口の δ_l ではなく κ 分解 δ_κ から組む
...
(同じ例で厳密に 1.0 を返す)。
⚠ 2026-08-15 時点では「ビット同一の制約は無い — f_x/f_e テーブルは未生成」だった。
260818Cl 訂正: f_x/f_e は **dataset-factors v1.0.0 として出荷済** (run 3、生成器
commit `0612e0c`、src/prod_factors_v1/MANIFEST.md) なので、この経路を変えると
出荷表と golden が動く。出荷済み F v5 とは今も非共有 (2026-08-15 に grep +
bitident 差分ゼロで確認)。"""
