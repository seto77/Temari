...
ReciPro との関係・取り込み判断の詳細は **ルートの `CLAUDE.md`** を参照。
...
| **球ベッセル Miller 規格化の 0/0 ガード** (`_jl_miller_scale`、2026-08-06) | **正しさ** | **未配備 — 要作者判断**。v3 出荷データは QC 済みで実害無しだが、handout で v3 世代を再生成する場合は本修正を入れた方がよい。窓の外はビット同一なので単独バックポート可能 |
