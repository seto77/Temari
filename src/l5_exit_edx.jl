# L5 Exit — 電子線入射 / F(s, E0) 出口
#
# docs/architecture.md の L5。ε 求積ノード、ε ノード 1 点分のドライバ、
# K 依存の N(K) 縮約、Bote-Salvat の絶対断面積、(Z, 殻, E0) パイプライン。
# 「何を積分して何を報告するか」を決めるのはこの層だけで、L0-L4 は出口を知らない。
#
# 新しい出口 (GOS / dσ/dΔE / δ_l など) を足すときは、L0-L4 に触れずにこの層の
# 隣に l5_exit_*.jl を足す形にする (docs/architecture.md「二つの軸」)。
