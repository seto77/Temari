...
#   → これがそのまま通常の弾性散乱位相シフト
#
# ⚠ 260818Cl: この一文は P3 時点の記録。ここまでが P3 の担当範囲で、δ_l から Mott の
#   dσ/dΩ・σ_el・σ_tr を出すのは P4 の未着手項目だった。それには
...
#
# ⚠ 260818Cl 訂正: 「交換は Slater の局所近似 (SCF が使っているのと同じ Xα 型) のみ」は
#   既定を純静電へ揃える前 (旧既定 `:xalpha`) の記述 — 下の ⚠⚠ を見ること。低エネルギー
...
戻り値は Dict (そのまま JSON 化できる):
  ...
  "sin2_delta"   sin²δ_l — σ_el = (4π/κ²)Σ(2l+1)sin²δ_l の材料
                 (260818Cl: P4 = `mott` 出口は κ 分解 δ_κ から別に組む)
  "l"            部分波の l
  "kappa_a0inv"  波数 κ [a₀⁻¹]
