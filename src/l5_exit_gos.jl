...
# 年代のもの (Egerton の SIGMAK/SIGMAL、Leapman の Hartree-Slater 表) しかない。
# ⚠ 260818Cl 訂正: 旧文「**近代的・相対論的な公開 GOS は事実上存在しない**」は誤り。
#   この出口に対する現代的な公開参照が 1 つある — Zhang らの Dirac GOS DB (2023、
#   Zenodo 7729585、CC-BY、Z=1–108、FAC の Dirac–Fock–Slater 軌道)。**我々は実際に
#   それと比較している** (docs の Verification ページ)。ただし q = 50 Å⁻¹
#   (s ≈ 3.98 Å⁻¹) で止まり、Bloch 波/マルチスライス用の形状因子表でもない。
#
# ---- 定義と、現行機構との対応 --------------------------------------------
...
#   * direct 項のみ (交換 −Re(DX*) は未実装)。260818Cl 訂正: 旧文「v4 の課題」は
#     出荷後も未着手 — 出荷 v4/v5 処方でも入っていない、処方の既知の限界
#   * ε 上端は運動学ではなく**利用者の選択**。GOS 自体に上限は無いので、
#     和則を見るときは Bethe 尾根 ε ≈ Q²/2 が ε 域に入っているか必ず確認すること
#   * 260818Cl 訂正: 旧文「相対論は放出電子のスカラー相対論 (--rel) まで」は v3
#     世代の記録。CLI の既定は 260809Cl から v4 = κ 分解 Dirac 連続状態 + 小成分
#     行列要素 (--rel = SRC は欠陥あり。docs/notes/src_defect_2026-08-07.md)。
#     ⚠ 関数 `compute_gos` の既定引数は基底 (v2) のまま — refcheck が関数既定に
#     依存しているため。既定処方を持つのは CLI の引数解釈だけ。
#     GOS の定義自体は非相対論の Born 形なので、γ 因子は掛けない
#     (掛けるのは断面積に直す側の仕事)
