...
#   端形状。多重項も固体 DOS も無いので白線の高さは出ない (設計上の対象外)。
...
全体 (既定 17 点、出荷版 321 点) を回さないので **F(s) より大幅に安い**。
260818Cl 訂正: 出荷版の点数は 161 → 321 (s≤8 の v3/v4 から s≤16 の v5 へ、
`gen_production.jl` の `S_GRID`)。
