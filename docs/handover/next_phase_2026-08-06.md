...
`docs/notes/speedup_audit_2026-08-05.json` の verdict 無し項目。**ビット破壊系 (l_need 打ち切り等) は
...
**推奨シナリオ: ~~3.1 の修正~~ → ~~P2 モジュール化 + CI~~ (どちらも 2026-08-06 完了) → P3 GOS 出口**

> **P2 の実施記録 (2026-08-06)**: `382f11a` で L0–L5 のファイル分割を実施。
> **Julia の `module` は導入せずフラット名前空間**のまま (作者判断) — include するだけで
> 全名前が見え、include 順に連結すれば単一ファイルへ戻せる。gen_production.jl / gui.jl /
> tools/*.jl は無改修で動作。純粋な移動なので 5 チャネル === でビット同一を確認済み。
> **演算子と出口の seam (注入可能化) は未実施** — 2 つ目の出口 (P3 GOS) が無いと
> 正しい形が決まらないため、そこまで持ち越すのが妥当と判断した。
> CI は作者が `.github/workflows/ci.yml` を用意済み (ubuntu/windows × 1.11.9/1.12 の
> selftest、bit-identity、refcheck ゲート)。分割後も参照先はすべて有効で、
> refcheck ジョブのコマンドはローカルで同一に通ることを確認した。
...
- 高速化の台帳: `docs/notes/speedup_audit_2026-08-05.md` / `.json` (43 案、27 が敵対的検証済み)
