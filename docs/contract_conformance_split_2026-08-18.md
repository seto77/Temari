...
⚠ **この一文は §6.2 で訂正した** — 実測は相対 1.49e-09 なので 1e-10 の検査でも捕まる。正しくは「**絶対**予算では捕まらない / 契約の相対検査で捕まる / 緩めれば余裕は削られる」
...
| `docs/factors_release_README.md` | 「非可逆な形で持つ場合」の節を 1 つ |
...
- **★ t = s² の取り違え (Cs)** は絶対 **2.44e-08 Å** = 予算 1e-7 の内側なので利用者側の
  精度検査を素通りし、相対では **1.49e-09** で契約の検査に引っ掛かる (負のテスト M2)。
  ⚠ ただし §2.2 の「相対 1e-12 **だけが**唯一の検出手段」は言い過ぎだった —
  1.49e-09 なら相対 1e-10 の検査でも捕まる。正しい言い方は
  「**絶対**予算では捕まらない / 契約の相対検査で捕まる / 許容を緩めれば余裕は削れる」
...
- **CI ジョブは足していない**。契約テストは出荷データ (`src/prod_factors_v1/`) を要るが、
  ...
  **変更が CI の対象 (Julia の selftest / ビット同一 / refcheck / small-component) に一切触れていない**
  ...
  契約テスト (既定 / A モード / 負のテスト) と `mkdocs build --strict`、`md_emphasis_check`
