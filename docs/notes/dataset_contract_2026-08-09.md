# データセットの仕様と完全性 — canonical を JSON に据える (2026-08-09)

*codex との 2 巡の議論 (`docs/notes/literature_findings_2026-08-12.md` の後続) を経た作者決定の実施記録。
...
### 1.4.1 ⚠⚠ 2026-08-13 の修正 — **このファイル自身が規約から外れていた**

C6 は**このファイルが実際に落ちていた罠**である。出荷側 C# `IonizationChannel.GridAt`
と突き合わせたら、E0 補間が **3 点**違っていた:

| # | 誤り | 旧 | 出荷 C# |
|---|---|---|---|
| 1 | 補間座標 | 生の `e0_keV` / 生の `F` | **x = ln(u−1)**、全正なら **y = log F** |
| 2 | 端点の傾き | 片側差分 `d[0]` / `d[-1]` | **3 点公式 + 単調性の制限** (`EdgeCase`) |
| 3 | 範囲外 | 端の値で clamp | **端区間の 3 次式で外挿** |

⚠ **#2 は E0 方向と s 方向の両方に効く** (同じ `Pchip` を両方に使うため)。

害は出荷 525 チャネル × 全 E0 中点 × 全 s ノード = 4,580,991 点で測った:
**最悪 |ΔF| = 2.8942e-03**、**最悪 相対 175 %** (符号が逆になる点がある)。
leave-one-out で比べると出荷規則 1.1832e-03 に対し素朴版は **2.5100e-03** (2.12 倍悪い)。
⚠⚠ **`check_tables` の C6 は出荷規則のほうを測っているので、素朴版の誤差は
どのゲートも見ていなかった。**

**直した後の検算は 2 通り**やった (片方だけでは自分の写しどうしの比較になる):
言語をまたぐ照合 (Python 参照 loader vs Julia 側 `Pchip` = 出荷 C# と同一実装) で
108 点 **4.5e-15**、および罠 C6 自身の負のテスト (素朴版と区別がつくこと)。

⚠ **出荷データは 1 バイトも変わっていない**。変わったのは参照 loader の返り値
(非ノードの E0 のみ)。詳細は `docs/handover/next_phase_2026-08-13.md` §2 P1c。
