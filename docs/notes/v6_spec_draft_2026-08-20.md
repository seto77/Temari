...
| 2 | 低 u 行の ε ノード増強 (Rn L1 @30 の 6.8e-06) | (b) 低 u 行だけ増強 / 全行 / 見送り | **判断の前に要実測** (u<2 の全行で改善量と費用を測る — 単一点 Rn L1 だけで閾値と倍率を決めない。codex) |
...
### (b) 低 u 行の ε ノード (n1/n2/n3) — **判断の前に要実測** (codex: 単一点根拠で採用しない)

- 事実: HIGH v6 の監査 (`../qcamp/audit_v6b.log`) で **Rn L1 @30 keV (u = 1.66) の ε ノード感度 6.8e-06**
  (n1/n2/n3 ×1.4 の振り)。部分波・角度・n_q は同行で 1e-09 以下。**v5 の監査でも同じ値 = 既知の項**で、
  v6 で新たに生じたものではない。量子化の半歩 5e-07 の約 14 倍
- **凍結前に測ること** (pilot 完走後の空き CPU で可能): u_min < 2 の全行 (E0 補間の危険域と同じ層。行数を先に数える)
  から層別標本を取り、ε ノード増強の改善量 (ΔF, ΔN0) と費用 (時間比) の分布を出す。閾値 (u < 2 か 1.5 か) と
  増強量はその分布から決める
- 採用する場合の設計 (codex): 倍率 ×1.4 でなく**実効値を明記** (例: (n1,n2,n3) = (20,56,20) → (28,79,28))。
  `u_min` の定義と境界の比較 (≤ か <) を仕様に書く。u が計算できない行は **fail-closed**。
  settings が行依存になるので、**宣言 (V6_SPEC) と行ごとの解決値 (resolved settings) を分離**し、
  JSON の settings には**実際に使った** (n1,n2,n3) と適用規則 ID と u_min を書く
- 見送る場合: errata / 誤差の内訳に「低 u 行の ε ノード感度 ≤ 7e-06 (既知、v5 と同水準)」を記載
