...
### 7.2 pilot (v4) — 完走 (2026-08-20 12:56:52。lane 完了 09:00〜12:56、9 レーン全部 attempt 1 / exit 0)

- **11 行 / 仕様内 192 窓 = 全合格 (192/192) + 仕様外 6**。指紋は最後まで **1 種 `0b10f74e9c4e398c`**
  (watchdog 再起動 0)。最悪 scaled **9.11e-08** (Ca M1 @400、[0.01, 1e5] eV、β = 200 mrad — v3 の最悪と同じ窓、
  v3 8.73e-08 と同じ床)、p90 8.18e-09 / 中央値 1.32e-09。v2 の不合格 5 窓はすべて床
  (Ca M1 [0,1e5] 4.58e-08 / [0,ε_max] 1.89e-08 ほか)。角度 縦 3.18e-15 / 横断 1.98e-15
- §2 の観測項目: (i) v2 の不合格 5 窓 → 床 ✓ (ii) **l_max_max が cap 256 に張り付いた窓 = 12** (その層の最悪
  9.11e-08 = 床。登録どおり打ち切り感度は P−O に出ない — `lkin_rule_study` の refA/refB から ≤ 4.1e-07 を引く)
  (iii) 行時間 中央値 5,414 s / 最大 17,921 s (Ca M1。序盤は v3 の残り 3 レーン等と並走)
- **v3 (:kappa_rc、cap 128) との比較: P−O はどちらも同じ床** — 窓求積 24 パネルが決めており、共有される
  部分波処方の差は登録どおり P−O に現れない。**F(s) (src、v6) と σ(β,Δ) が 1 つの部分波処方を共有する最終候補
  v4 が pilot を通過** ⇒ deep (525ch × E₀ 3 点、≈ 14〜24 日) の起動は作者判断
- 集計 = `../qcamp/cert_v4_pilot_report_final.txt` (Python) / `cert_v4_pilot_summary_final.txt` (Julia)。
  JSONL・ログ = `../qcamp/cert_runs/pilot_v4/` へ移動 (2026-08-20 昼)
- **§0-4 の観測 (n_q 720 vs 1216) — 実施済 (2026-08-20 13:3x、`../qcamp/nq720_vs_1216.log`)**: 旧不合格 5 窓
  × β=200 で、V4 と「n_q=720 だけ変えた変種」の σ_own の相対差 = **Ca M1 @400 −3.59〜−3.63e-07 (4 窓とも一致) /
  Zn M3 @300 −7.7e-08**。既知の直接測定 (n_q 1216 の補間影響 4.74e-07、8 条件) と同じ桁 — 数値誤差の内訳に
  並記する共有処方の項で、P−O には出ない。**合否には足さない** (登録どおり観測のみ)
