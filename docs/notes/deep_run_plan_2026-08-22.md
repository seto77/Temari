...
- **(b) 遅い機は尾を掴まない (claim 側の規則)** — 各ホストは**自分の**サイドカーから自分の相対速度を
  ...
  **F v6 完走後の確定値 (2026-08-23)**: 実測済 13 host の slowdown の中央値は **1.81**、
  **K=1, X=1.30** とする。従って 36 slots 構成では残票 **<36** のとき、slowdown **>2.353** の
  host だけが standby する（F v6 の d317-5=2.38 と m616-2=3.46）。同じ完走 manifest を start 時刻順に
  数えると、この境界内で両者が掴んだのは 5 票、計 **1,258.3 min**（m616-2 1票 188.5 min、d317-5
  4票 1,069.8 min）。X=1.5 は m616-2 の 1票しか防げず、X=1.25 は同じ5票を止めるため、境界を
  d317-5 の 2.38 より僅かに下げずに済む X=1.30 を採る。d317-10 は退役、d317-1 は未測定なので
  speed map へ入れず fail-open のままにする。実装は `control/tail.json` (PROTOCOL §5.8) で、再登録後の
  worker_id と F v6 sidecar の対応を事前登録時に再確認する。高速 host 不在時の永久停止を避けるため
  `rescue_seconds=600` 後は 1 票を claim する（他 host/NAS の時計は使わない）。
