...
| `--kli` | Replace local Xα exchange with **exact exchange** in the KLI form, and drop the Latter correction — the $-(Z-N+1)/r$ tail then comes out of the physics. Adds `-KLI` to the model id. Only wired into the non-relativistic SCF, so it implies `--nodscf` and says so when it does. Costs roughly 2–3× the SCF time (Au 21 s, once per element, then cached). See [Physics](physics.md#exact-exchange-kli) for what it buys. |
