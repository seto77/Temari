...
| `julia -t auto tools/small_component_check.jl [--scan]` | The small-component fraction against the only external figures that exist for it, **gated in CI**. The reference carries one significant digit, so the criterion is whether our value falls inside the interval that rounds to it — not a ratio band, which would admit a factor of five |
...
- the small-component fraction against the external one-digit figures, which needs
  no reference data on disk
