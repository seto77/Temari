...
| **部分断面積 σ(β, Δ)** | 中 | EELS 定量の k 因子そのものです。このリストの中で最も応用範囲の広い量です。実際の作業はエネルギー窓に対する ε ノードの設計です。 |
...
| **Mott 弾性 dσ/dΩ、σ_el、σ_tr** | **done** | `mott` サブコマンドです。スピンが入っています。κ 分解 Dirac 連続状態が $\delta_\kappa$ を与え、そこから直接振幅 $f(\theta)$ とスピン反転振幅 $g(\theta)$ の両方、Sherman 関数 $S(\theta)$、そして $\sigma_\text{el}$、$\sigma_\text{tr}$ が得られます。NIST SRD 64 (Powell et al., 2016) に対する比は 1 keV 以上で 0.90–0.94 です。⚠ 散乱ポテンシャルは純静電のままにしなければなりません — 標的自身の Xα 交換を足したものは入射電子が感じる場ではなく、$\sigma_\text{el}$ を NIST の 1.6–4.9 倍に膨らませます。 |
