...
| 検証 | selftest ALL PASS / refcheck WORST 9.044e-08 / verify_simd_bessel 288 ケース / E5 単体オラクル 75 ケース |
...
### 3.1 ★唯一の未修正の正しさ欠陥 — 球ベッセル Miller 正規化 (j₀≈0 で 0/0)

**最優先。次フェーズの最初の 1 手にすること。**

- **場所**: `src/ionization.jl:397` (スカラー版 `scale = (sin(x)/x) / out[1]`) と
  `src/ionization.jl:427` (`_scale8`、8 レーン SIMD 版が同式)。**両方直す必要がある**
- **症状**: Miller の生値は任意スケールなので、j₀(x) ≈ 0 となる x ≈ nπ で raw_j₀ が丸め誤差そのものになり、
  スケール係数が壊れて全 λ が汚染される。誤差は 1e-16/|sin x| で効く
  (実測: |x−nπ| = 0 で相対誤差 0.06–1.9 = 破綻、1e-9 で 1e-7、1e-6 で 5e-10)
- **v3 への実害は無い**: 5×10¹⁰ 回の評価で被弾 ~320 回、1 回の被弾は動径 5000 点中 1 点を 1e-7 ずらすだけで
  R 積分への影響は 1e-11 相対
- **推奨修正 = 閾値ガード方式**:
  ```julia
  s, c = sincos(x); j0 = s/x
  scale = abs(j0) >= J0_MIN ? j0/raw0 : ((j0 - c)/x)/raw1   # J0_MIN は 1e-8 級
  ```
  計画書 §8.1 の原案は「j₀ と j₁ の大きい方で規格化」だが、**それだと全域で選択が変わり得て
  ビット同一を広く失う**。閾値ガードなら**現行が壊れている窓の外ではビット同一**を保てるので、
  v4 全再生成を待たずに入れられる。§6 の「決定論バグの修正はビット互換に優先する」原則にも適合
- **検証**: (a) selftest T0 に x = nπ ± {0, 1e-12, 1e-9, 1e-6} のケースを追加、
  (b) `tools/verify_simd_bessel.jl` 288 ケースでスカラー/SIMD 一致、
  (c) 5 チャネル === 比較で「窓の外はビット同一」を実証、(d) refcheck 不変
...
**推奨シナリオ: 3.1 の修正 (半日) → P2 モジュール化 + CI → P3 GOS 出口**
...
- **ビット同一の検証手順**: `selftest` (~10 s) → `refcheck` (~1 分) → 5 チャネル === 比較 →
