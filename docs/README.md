...
| [`instructions_publish_agreement_2026-08-22.md`](handover/instructions_publish_agreement_2026-08-22.md) | **Work instruction (for a codex chat): make publish judge a name collision by agreement rather than by bytes.** Why the byte rule stopped meaning anything once bit identity across CPUs was given up (2026-08-21), the three conditions under which it fires — gen_production only, a reaped worker that was actually alive, two machines in different equivalence classes — and the rule that replaces it, with “cannot judge” never counting as agreement. The adjudicator is the existing `tools/agreement_check.py`, unchanged and already inside every code archive; workers get Python at registration alongside Julia. Carries an earlier draft's reversal in its header. |
